...
$qery = "SELECT s.student_name, s.student_id, c.class_id from student as s, class as c where s.student_id='$rollid' and c.class_id=s.class_id";
$sql = mysqli_query($qery);
$i = mysqli_num_rows($sql);
if($i>0){
	while ($row = mysqli_fetch_assoc($sql)) {
		?>
		<p><b>Student Name :</b> <?php echo $row['student_name'];?></p>
		<p><b>Student Roll Id :</b> <?php echo $row['student_id'];?></p>
		<p><b>Student Class:</b> <?php echo $row['class_name'];?></p>
<?php		
	}
...
// Code for result
 ...
 $query ="SELECT t.student_name,t.student_id,t.class_id,t.marks,SubjectId,course.course_name from (select sts.student_name,sts.student_id,sts.class_id,tr.marks,course_id from student as sts join  result as tr on tr.student_id=sts.student_id) as t join course on course.course_id=t.course_id where (t.student_id=:rollid and t.class_id=:classid)";
...
<td style="text-align: center"><?php echo htmlentities($result->course_name);?></td>
...
?>
                                    ...
                                    <div class="form-group">
                                                            ...
                                                            <div class="col-sm-6">
                                                               <a href="login-student.php">Back to Home</a>
                                                            </div>
                                                        </div>
